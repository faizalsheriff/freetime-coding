 ...
 */
public class AstCQLClient2 {
  private static final Logger logger = LoggerFactory.getLogger(AstCQLClient2.class);
  
  private AstyanaxContext<Keyspace> context;
  private Keyspace keyspace;
  private ColumnFamily<Integer, String> EMP_CF;
  private static final String EMP_CF_NAME = "employess123";
    ...
    try {
      OperationResult<CqlResult<Integer, String>> result
        = keyspace.prepareQuery(EMP_CF)
          .withCql(String.format("SELECT * FROM %s WHERE %s=%d AND %s=%d;", EMP_CF_NAME, COL_NAME_EMPID, empId))
          .execute();
  ...
  public static void main(String[] args) {
    logger.debug("main");
    AstCQLClient2 c = new AstCQLClient2();
    c.init();
    //c.createCF();
