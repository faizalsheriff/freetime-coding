...
public class Crc32 {
	...
	static final int CRC32TABLE[] = { 0x00000000, 0x77073096, 0xee0e612c,
	...
	public final static int value(int crc) {
		return ~crc;
	}

	public final static int update(final int crc, final int b) {
		int c = crc;
		return (c >>> 8) ^ CRC32TABLE[(c ^ b) & 0xFF];
	}

	public final static int updateInt(final int crc, final int i) {
		int c = crc;
		c = update(c, (i >>> 24) & 0xFF);
		c = update(c, (i >>> 16) & 0xFF);
		c = update(c, (i >>> 8) & 0xFF);
		c = update(c, i & 0xFF);
		return c;
	}

	public final static int update(final int crc, final byte[] buffer) {
		return update(crc, buffer, 0, buffer.length);
	}
	...
	public final static int update(final int crc, final byte[] buffer,
		...
		int c = crc;
		for (int i = 0; i < len; i++) {
			c = (c >>> 8) ^ CRC32TABLE[(c ^ buffer[i]) & 0xFF];
		}
		return c;
	}
}
