...
public class BlockSyncTest extends ProfileTest {
	...
	protected void onSetup(Object source, ComponentSetupArgs args)
		...
		sm.register(new StateManagerListenerImpl() {
			...
			public void stateManagerProcessMessage(StateManager sm,
				...
				super.stateManagerProcessMessage(sm, message);
				BlockSyncTest.this.processStateManagerProcessMessage(message);
			}

			public void stateManagerProcessMessageComplete(StateManager sm,
					StateManagerMessage message) {
				super.stateManagerProcessMessageComplete(sm, message);
				BlockSyncTest.this
	...
	protected void onRun(ProfileResult result) {
		...
		if (newEvent) {
			...
			currResult.setValue(totalTime / totalSamples);
	...
	public void processStateManagerProcessMessage(StateManagerMessage message) {
		if ((message.getType() == StateManagerMessage.TYPE_SYNCHRONIZE
				&& sm.getStatus() != StateManagerStatus.MASTER)
		|| (message.getType() == StateManagerMessage.TYPE_BLOCK_REQUEST
				&& sm.getStatus() == StateManagerStatus.MASTER)) {
		...
		try {
			...
			|| (message.getType() == StateManagerMessage.TYPE_BLOCK_REQUEST
				...
				if (startTime != 0) {
					...
					totalTime += currTime;
					//currResult.setNotes("BLOCK SYNC");
					if (maxTime < currTime) {
						maxTime = currTime;
					}
			...
			UnitManager.Logging.logSevere("BlockSyncTest exception: " + e);
		}
	}
}
